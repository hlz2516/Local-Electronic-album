...
public class CoverUI {
	public static BorderUI create() {
		...
		func.setLayout(new FlowLayout(FlowLayout.CENTER, 20, 2));
		
		Dimension btnSize = new Dimension(120, func.getPreferredSize().height - 10);
		
		JLabel message = new JLabel();
		
		JButton createbtn = new JButton("创建相册");
		...
		JButton savebtn = new JButton("保存相册");
		
		JButton deleteBtn = new JButton("删除相册");
		deleteBtn.setPreferredSize(btnSize);
		deleteBtn.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
			  if(Cover.getCurCover() == null) {
				  message.setText("请先选中一张相册簿封面");
				  return;
			  }
				//弹出对话框提示用户确认是否删除
